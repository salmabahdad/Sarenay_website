...
if ($_SERVER["REQUEST_METHOD"] == "POST") { 
    ...
    // Vérifiez les données d'identification de l'utilisateur (par exemple, vérifiez dans la base de données)
    $email = $_POST['email'];
    $password = $_POST['password'];

    // Vérifiez ici si l'email et le mot de passe correspondent à un utilisateur dans la base de données
...
?>
    ...
    <div class="relative  h-36  " >
        ...
        <a href="login.html" class="font-thin mt-2.5 text-sm hover:text-gray-600  text-gray-400"><img class="h-4 mt-0 ml-auto"src="images/user.png"></a>
 ...
 <div class="  min-h-auto flex justify-center relative w-full bg-gray-50   items-center px-4">
  <!-- Left Section (Login) -->
  <div class="max-w-3xl bg-gray-50 p-8  text-gray-800">
      <h2 class="text-lg font-mono mb-4 text-black">ALREADY REGISTERED ?</h2>
      <p class="text-xs font-serif text-gray-600">If you're already a member of our community, please proceed by logging in.</p><br>
      <form name="login" method="post">
          <div class="">
              <label for="email" class="block text-sm font-serif text-gray-500">Email Address *</label><br>
              <div class="custom-input-wrapper">
  <input type="email" class="custom-input" name="email" placeholder="Email" aria-required="true" required >
</div>
<p class="error text-red-600 text-xs"><?php echo isset($errors['email']) ? $errors['email'] : ''; ?></p><br>
          </div>
          <div class="">
            ...
            <p class="error text-red-600 text-xs"><?php echo isset($errors['password']) ? $errors['password'] : ''; ?></p><br>
            
          </div>
          <button type="submit"
              class="w-full bg-black text-white py-2 px-4  hover:bg-gray-900 focus:outline-none focus:bg-gray-900">LOG IN</button>
      </form><br>
      <a href="admin.php" class=" text-sm font-serif text-gray-700 hover:text-gray-950">You're an admin ?</a>
  </div>

  <!-- Separator Line -->
  <div class="w-0.5 bg-gray-300 mx-8 h-96 hidden sm:block"></div>

  <!-- Right Section (Sign Up) -->
  <div class=" container max-w-sm w-full p-8 ">
    
      <h2 class="text-lg  font-mono mb-4 text-black">CREATE YOUR ACCOUNT</h2>
    
     <p class="text-base font-serif text-gray-600">With a Sarenay account, effortlessly view your order history and curate your wishlist with a single click.</p> 
     <br>
     <input class="bg-black w-52 h-10 text-white hover:bg-gray-900 focus:outline-none focus:bg-gray-900" type="button" value="SIGN IN" onClick="signinpage()">
  </div>
